...
class ArubaActivate
{
	...
	public function __construct( $OPTIONS = array() )
	{
		...
		if ( !isset($this->options["username"]) ) { $this->options["username"] = ARUBA_ACTIVATE_USER; }
		if ( !isset($this->options["password"]) ) { $this->options["password"] = ARUBA_ACTIVATE_PASS; }
	...
	public function authenticate()
	{
		...
		$POSTARRAY = array();
		foreach($POST as $KEY => $VALUE)					// This could be done in 1 line and alot simpler...
		{
		...
		if ( preg_match_all($REGEX, $RESPONSE, $MATCHES) )	// Look for any and all cookies that come back
		{
			foreach($MATCHES[1] as $OMNOMNOM)				// This lets us see all the cookies
			{
				$this->curlopts[CURLOPT_COOKIE] = $OMNOMNOM;// Eventhough we only store the last one for now
			}
		}
		if ( !isset($this->curlopts[CURLOPT_COOKIE]) ) { print "AUTH ERROR: {$RESPONSE}\n"; }
	...
	public function get_devices_by_mac($MACS)			// This should be an array of mac addresses...
	{
		...
		$QUERY = array(									// Build a query assoc array... This format
