...
class CloudflareDNSClient
{
    ...
    {
        if (!$email) {
            throw new \Exception('Missing account email address');
        }
        if (!$apikey) {
    ...
    public function addZoneRecord($zone, $type, $name, $content, $ttl = 120)
    {
        ...
        if (!$zoneid) {
            throw new \Exception("could not determine zone id for {$zone}");
        }
    ...
    {
        if (!$zoneid) {
            throw new \Exception('fetchRecords called with blank zone ID');
        }
        $uri = $this->baseuri.'/zones/'.$zoneid.'/dns_records';
